...
# sglang-turboquant image: liuhuijiayou/sglang feat/turboquant-kvcache
# (PR #23135 against sgl-project/sglang main). Targets sm_86 + sm_89 only.
...
# (KT-kernel build dropped — TurboQuant doesn't need MoE CPU expert offload.)
...
# SGLANG_VERSION at build time; without it the version falls back to
# 0.0.0.dev0 which can confuse package-version probing downstream.
ENV SGLANG_VERSION=0.6.1
WORKDIR /opt/sglang
