...
# ----------------------------------------------------------------------
# Local patch: stub out kernel symbols missing from our source-built
# sgl-kernel (FP4 + GPTQ-Marlin repack, both Blackwell-only / unbuilt
# variants). Many sglang model files do 
# at module top level; without these stubs the entire model registry
# fails to load anything that touches FP4 or GPTQ-Marlin code paths.
# Setting them to None lets the imports succeed; downstream call sites
# already null-check (we verified standard.py, modelopt_quant.py).
# ----------------------------------------------------------------------
for _missing in (
    "scaled_fp4_quant",
    ...
    if _missing not in globals():
        globals()[_missing] = None
del _missing
