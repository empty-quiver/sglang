...
def _input_supports_flashinfer_norm(input: "torch.Tensor") -> bool:
    """Return True only if the input tensor lives on a CUDA device with
    compute capability >= sm_89. flashinfer's JIT-compiled norm kernel
    is built for sm_89 only on this host (Dockerfile sets ARCH 8.6;8.9
    but flashinfer's bundled .so only includes sm_89 cubins). On sm_86
    devices (RTX 3060) the call fails with `no kernel image is available
    for execution on the device`; in that case the caller should fall
    back to _gemma_rmsnorm_internal / _rmsnorm_internal which routes
    through sgl_kernel's common_ops.so where we have sm_86+sm_89."""
    import torch as _torch
    if input.device.type != "cuda":
        return False
    try:
        cap = _torch.cuda.get_device_capability(input.device)
    except Exception:
        return False
    return cap >= (8, 9)
...
def rmsnorm(
    ...
    if (
        ...
        and _input_supports_flashinfer_norm(input)
    ):
